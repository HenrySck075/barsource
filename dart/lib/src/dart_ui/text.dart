...
class Locale {
  /// Creates a locale from a language code and an optional country code.
  ///
  /// The [languageCode] argument must not be null. It should be a
  /// lowercase two-letter code as defined by ISO 639-1. The [countryCode]
  /// argument is optional, but if provided, it should be an uppercase
  /// two-letter code as defined by ISO 3166-1 alpha-2.
  ...
  /// The language code, such as "en" for English or "es" for Spanish.
  final String languageCode;

  /// The script code, such as "Latn" for Latin or "Cyrl" for Cyrillic.
  final String? scriptCode;

  /// The country code, such as "US" for the United States or "BR" for Brazil.
...
final class GlyphInfo {
  ...
  // stored in the same order as the constructor above
  static GlyphInfo? _packedArray(Pointer<Int32> ptr) {
    if (ptr == nullptr) {
      return null;
    }
    final left = ptr.cast<Float>()[0];
    final top = ptr.cast<Float>()[1];
    final right = ptr.cast<Float>()[2];
    final bottom = ptr.cast<Float>()[3];
    final graphemeStart = ptr[4];
    final graphemeEnd = ptr[5];
    final isLTR = ptr[6] != 0;
    return GlyphInfo._(left, top, right, bottom, graphemeStart, graphemeEnd, isLTR);
  }
...
Int32List _encodeParagraphStyle(
  ...
  if (fontSize != null) {
    result[0] |= 1 << 8;
    realResult.setFloat32(7*4,fontSize,_kFakeHostEndian);
  }
  ...
  if (height != null && height != kTextHeightNone) {
    result[0] |= 1 << 9;
    realResult.setFloat32(8*4,height,_kFakeHostEndian);
  }
  if (strutStyle != null) {
    result[0] |= 1 << 10;
    // Passed separately to native.
...
base class _NativeParagraph implements Paragraph {
  /// This class is created by the engine, and should not be instantiated
  /// or extended directly.
  ///
  /// To create a [Paragraph] object, use a [ParagraphBuilder].
  _NativeParagraph._(this._nativePtr);
  ...
  List<TextBox> _decodeTextBoxes(Pointer<Float> pointer) {
    final int count = pointer.cast<Uint32>()[0] ~/ 5;
    final encoded = (pointer+1).asTypedList(count);
    final boxes = <TextBox>[];
    var position = 0;
...
base class _NativeParagraphBuilder implements ParagraphBuilder {
  ...
  _NativeParagraphBuilder(ParagraphStyle style)
    ...
    final Uint8List essBuffer = encodedStrutStyle?.buffer.asUint8List() ?? Uint8List(0);
    using((arena) {
    _nativePtr = rina_paragraph_builder_create(
      style._encoded.address,
      essBuffer.address,
      encodedStrutStyle!=null,
      (style._fontFamily ?? '').asNativePointer(arena).cast<Char>(),
      _createStringArray(strutFontFamilies, arena),
      strutFontFamilies?.length ?? 0,
  ...
  Pointer<Pointer<Char>> _createStringArray(List<String>? list, Arena arena) {
    if (list == null) return nullptr;
    final buffer = arena<Pointer<Char>>(list.length);
    for (var i = 0; i < list.length; i++) {
      buffer[i] = list[i].asNativePointer(arena).cast<Char>();
    }
    return buffer;
  }

  late final Pointer<TennojiParagraphBuilder> _nativePtr;
  ...
  void pushStyle(TextStyle style) {
    ...
    if (fontFeatures != null) {
      encodedFontFeatures = ByteData(4 + fontFeatures.length * FontFeature._kEncodedSize);
      encodedFontFeatures.setUint32(0, 0);
      var byteOffset = 4;
      ...
      encodedFontFeatures = ByteData(4);
      encodedFontFeatures.setUint32(0, 0);
    }
    ...
    if (fontVariations != null) {
      encodedFontVariations = ByteData(4 + fontVariations.length * FontVariation._kEncodedSize);
      encodedFontVariations.setUint32(0, 0);
      var byteOffset = 4;
      ...
      encodedFontVariations = ByteData(4);
      encodedFontVariations.setUint32(0, 0);
    }

    final handynullptr = nullptr.cast<TennojiCanvasPaintMetadata>();
    using((arena)=>
    rina_paragraph_builder_push_style(
      _nativePtr,
      encoded.address,
      _createStringArray(fullFontFamilies, arena),
      fullFontFamilies.length,
      style._fontSize ?? 0,
      style._letterSpacing ?? 0,
      style._wordSpacing ?? 0,
      style._height ?? 0,
      style._decorationThickness ?? 0,
      style._locale?.toString().asNativePointer(arena).cast<Char>() ?? nullptr,
      style._background?._createPaintMetadata(arena) ?? handynullptr,
      style._foreground?._createPaintMetadata(arena) ?? handynullptr,
      Shadow._encodeShadows(style._shadows).buffer.asUint32List().address,
      encodedFontFeatures.buffer.asUint32List().address,
      encodedFontVariations.buffer.asUint32List().address,
    )
    );
  }

  @override
  void pop() => rina_paragraph_builder_pop(_nativePtr);

  @override
  void addText(String text) {
    using((arena){
      rina_paragraph_builder_add_text(
        _nativePtr, 
        text.asNativePointer(arena).cast<Char>()
      );
    });
  }
  ...
  void addPlaceholder(
    ...
    rina_paragraph_builder_add_placeholder(
      _nativePtr,
      width * scale,
      height * scale,
      alignment.index,
